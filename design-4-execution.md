...
**Implementation Pattern:**
```typescript
// ❌ Wrong - input blends with background, unclear affordance
<Textarea
  className="flex-1 min-h-[150px] resize-none"
  placeholder="Enter feedback..."
/>

// ✅ Correct - white background provides clear "type here" signal
<Textarea
...
**UX Principle:**
```typescript
// ❌ Wrong - arbitrary minimum blocks submission, creates friction
if (textFeedback.length < 50) {
  setError('Please write at least 50 characters of feedback before finishing.');
  return;
}

// ✅ Correct - allow any feedback, encourage more on next screen
...
**Implementation Pattern:**
```typescript
// ❌ Wrong - same generic text for all modes
<Button onClick={handleFinish}>
  {isFinishing ? 'Finishing...' : 'Finish Test'}
</Button>

// ✅ Correct - context-specific text for each mode
<Button onClick={handleFinish}>
  {isFinishing
    ? 'Saving...'
...
// ✅ Correct - shows feedback once in editable form
<textarea
...
**Example from task**:
```tsx
// ❌ Wrong - fixed positioning overlaps with progress bar
<div className="fixed top-0 left-0 right-0 z-50 bg-amber-500">
  Preview Mode Banner
</div>

// ✅ Correct - normal document flow pushes content down
...
- [x] Test screen (testing-interface.tsx) ← Easy to miss
...
5. **Status.md Workflow**: Correctly moved task from "Ready to Execute" → "Testing" section
